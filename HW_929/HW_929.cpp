#include <iostream>
        ...
        public:
            ...
            bool isPotentialVictory(TicTacToe::Entry computer) //check if there is already two o in a row/column/diagonal
            {
                // test first col
                if (board[0][0] == (char)computer && board[0][1] == Entry::EMPTY && board[0][2] == Entry::EMPTY)
                {
                    return true;
                }

                else if (board[0][2] == (char)computer && board[0][1] == Entry::EMPTY && board[0][0] == Entry::EMPTY)
                {
                    return true;
                }

                else if (board[0][2] == (char)computer && board[0][0] == Entry::EMPTY && board[0][1] == Entry::EMPTY)
                {
                    return true;
                }
        ...
        static bool getComputerMove(TicTacToe::Entry computer, TicTacToe& t)
        {
            ...
            do {
                ...
                if (t.isPotentialVictory(computer) == true) //call isPotentialVictory and if it returns true
                {
                    if (t.board[0][0] == (char)computer && t.board[0][1] == (char)computer && t.board[0][2] == t.EMPTY)
                    {
                         t.board[0][2] = char(computer); //fill empty spot in col 1
                    }

                    else if (t.board[0][2] == (char)computer && t.board[0][1] == (char)computer && t.board[0][0] == t.EMPTY)
                    {
                        t.board[0][0] = char(computer); //fill empty spot in col 1
                    }

                    else if (t.board[0][2] == (char)computer && t.board[0][0] == (char)computer && t.board[0][1] == t.EMPTY)
                    {
                        t.board[0][1] = char(computer); //fill empty spot in col 1
                    }
                }
                else
                {
